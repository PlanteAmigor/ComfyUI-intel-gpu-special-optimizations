...
class KSAMPLER(Sampler):
    ...
    def sample(self, model_wrap, sigmas, extra_args, callback, noise, latent_image=None, denoise_mask=None, disable_pbar=False):
        ...
        if callback is not None:
            def _cooled_callback(state):
                nonlocal _step_times, _step_t0
                i = state["i"]
                # Call original callback first
                callback(i, state["denoised"], state["x"], total_steps)

                # ---- GPU 冷却：每步休息 3 秒 ----
                if i < total_steps - 1:
                    step_elapsed = time.time() - _step_t0
                    if len(_step_times) >= 3:
                        _step_times.append(step_elapsed)
                        if len(_step_times) > 20:
                            _step_times.pop(0)
                        median_t = sorted(_step_times)[len(_step_times)//2]
                        if step_elapsed > median_t * 2.0:
                            print(f"  🌡️  ComfyUI 降频（{step_elapsed:.2f}s > 基线{median_t:.2f}s × 2），额外冷却 10 秒...")
                            time.sleep(10)
                    _step_t0 = time.time()
                    gc.collect()
                    if hasattr(torch, 'xpu') and torch.xpu.is_available():
                        torch.xpu.empty_cache()
                    print(f"  💤 ComfyUI 冷却：第 {i+1}/{total_steps} 步，休息 3 秒...")
                    time.sleep(3)

            k_callback = _cooled_callback
